...
void RobotContainer::ConfigureBindings() {
  ...
  /** driverJoystick.X().WhileTrue(drivetrain.ApplyRequest([this]() -> auto && {
     if (frontLimeLight.LLHasTarget) {
       ...
       return drive.WithRotationalRate(-driverJoystick.GetRightX() * MaxAngularRate);
     }
   }));*/
   ...
   driverJoystick.B().WhileTrue(drivetrain.ApplyRequest([this]() -> auto && {
     return point.WithModuleDirection(
         frc::Rotation2d{-driverJoystick.GetLeftY(), -driverJoystick.GetLeftX()});
   }));
...
void RobotContainer::ConfigureDashboard() {
 ...
 if(systemJoystick.Start().Get() == true){ if(subsystemClaw.hasCoral()){
  ...
  if(systemJoystick.POVUp().Get() == true) subsystemClaw.setAxis(90_deg);
    if(systemJoystick.POVLeft().Get() == true) subsystemClaw.setAxis(20_deg);
        if(systemJoystick.POVDown().Get() == true) subsystemClaw.setAxis(0_deg);
        if(systemJoystick.POVRight().Get() == true) subsystemClaw.setAxis(150_deg);
 ...
 if(systemJoystick.X().Get() ==true) subsystemElevator.setHeight(2.3_tr);// 2.5 tr = l3 at 30 deg  
 //4tr? l4 90 deg
 if(systemJoystick.Y().Get() ==true){
  subsystemClaw.setAxis(40_deg);
   subsystemElevator.setHeight(4.55_tr);}
 if(systemJoystick.RightBumper().Get() == true) subsystemClaw.percentOut(-0.2);
  if(systemJoystick.LeftBumper().Get() == true) subsystemClaw.percentOut(0);
