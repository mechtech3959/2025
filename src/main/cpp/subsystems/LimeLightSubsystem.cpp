...
frc::Pose2d LimeLight::poseEst(std ::string name) { if(LLHasTarget && (tx < 0.1 || tx > -0.1)){
  std::optional<LimelightHelpers::PoseEstimate> posEst = LimelightHelpers::getBotPoseEstimate_wpiBlue_MegaTag2(name);
  frc::Pose2d p =  (posEst.has_value())?  frc::Pose2d{posEst->pose} :  frc::Pose2d{0_m,0_m,0_deg};
  return p;
